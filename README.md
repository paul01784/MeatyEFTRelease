
# MeatyEFTRelease

Advanced external DMA radar and companion toolkit for **Escape from Tarkov** focused on performance, clean visuals, and live data features.

You can find out more on https://discord.gg/GXmBagdP5s and get support, pass on suggestions etc etc
This is a DMA Radar and Fuser support is included but not required. There are no memory writing included in this release and is considered as safe as can be if your firmware is upto date

Please note, dlcks will be banned or ignored, use your brain

---

# Credits

There are a lot of credits to go about. All of this information has come from unknowncheats, earwigging, discord posts, and other public sources, including DMA Educational Resources, so credit goes to x0m, mamboo and co.
Im pretty certain the maps in this project came from Marazm, so credit to this guy for doing maps for the general public to use.

---

# Building

I've tried to make this work easy. You should be able to download, open is visual studio, select release then build. If you have issues you can ask in the Discord or Google it

To get the assets needed after building to run you can download them here
https://www.dropbox.com/scl/fo/ww5zu4nxm5f4w96vbgdwm/ADMHHuq2uaEGMbRHIxcF5uk?rlkey=bcik8g428iadfow3bgjzmdchj&st=lf5twyrg&dl=0

You should acquire your own vmm.dll, leechcore.dll and FTD3XX.dll but i have included these if you require

NOTE THIS SHOULD BE RAN ON YOUR 2nd PC AND NOT GAME PC
---

# Status

Active development & Updates will be published

Features, offsets, and integrations may require maintenance after game updates.

---

# Features

- Real-time radar rendering  
- Player tracking  
- Loot filtering & wishlist system  
- Quest helper / objective markers  
- Extract locations  
- Group detection  
- Map zoom / pan / follow modes  
- Live player data integration  
- Dogtag database integration  
- Fast scatter-read architecture  
- Optimised C++ performance build  

---

# Tech Stack

- C++  
- DirectX9  
- ImGui  
- GLM  
- nlohmann::json  
- libcurl  

---

# License

This project is released under the GNU GPL v3 License.

You may use, study, and modify this code under the terms of GPLv3.

If you distribute modified versions, source code must also be made available under the same license.

---

# Important Notice

This repository contains client-side source only.

Production infrastructure including:

- Authentication systems
- API endpoints
- Security tooling
- Backend databases

are proprietary and are not included in this repository. Users are not required to signup or use this but is provided free to anyone

Unauthorised use of production services is prohibited.

---

# Build Notes

Visual Studio recommended.

Typical dependencies:

- DirectX SDK / Windows SDK
- vcpkg packages
- C++ latest standard enabled

---

# Disclaimer

This project is provided for educational and research purposes only.

Use at your own risk.

---

# Contact

For legitimate enquiries, collaboration, or licensing discussions, contact privately.

You can also find me on discord server https://discord.gg/GXmBagdP5s
