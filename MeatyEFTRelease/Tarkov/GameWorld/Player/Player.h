...
public:
	...
	bool isDead;
	bool hasExfiled;
	std::uint32_t consecutiveRosterMisses{ 0 };
	std::chrono::steady_clock::time_point rosterMissingSince{};
	...
	// Default constructor
	Player(PlayerKind playerKind = PlayerKind::Unknown)
		: instance(0),
		kind(playerKind),
		P_HandsController(0),
	...
	void setKind(PlayerKind playerKind) noexcept
	{
		kind = playerKind;
	}

	[[nodiscard]] PlayerKind getKind() const noexcept
	{
		return kind;
	}
};

using PlayerCollection = std::vector<Player>;
using PlayerSnapshot = std::shared_ptr<const PlayerCollection>;
