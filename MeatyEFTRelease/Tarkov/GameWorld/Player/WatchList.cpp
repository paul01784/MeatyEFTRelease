...
bool WatchListManager::AddPlayer(Player& player, const std::string& reason)
{
...
void WatchListManager::UpdateWatchStatus(Player& player) const
{
    const std::string profileId = ResolveProfileId(player);
...
bool WatchListManager::logAddPlayer(Player& player)
{
    std::unique_lock<std::shared_mutex> lock(mutex_);
...
bool WatchListManager::logUpdatePlayerPID(Player& player)
{
    std::unique_lock<std::shared_mutex> lock(mutex_);
...
bool WatchListManager::UpsertRaidPlayerUnlocked(Player& player, bool requireProfileId)
{
    if (!activeRaidIndex_.has_value())
        return false;
...
void WatchListManager::CopyPlayerDataUnlocked(RaidPlayerEntry& destination, const Player& player) const
{
    const std::string profileId = ResolveProfileId(player);
...
bool WatchListManager::IsEligiblePlayer(const Player& player)
{
    return player.isPlayer &&
        !player.isPlayerScav;
}

std::string WatchListManager::ResolveProfileId(const Player& player)
{
...
std::string WatchListManager::ResolveAccountId(const Player& player)
{
...
std::string WatchListManager::ResolvePlayerName(const Player& player)
{
