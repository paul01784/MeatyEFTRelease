...
void to_json(nlohmann::json& j, const radarGlobals& r) {
    j = nlohmann::json{
        ...
        {"drawAimLineTargets", r.drawAimLineTargets},
...
void from_json(const nlohmann::json& j, radarGlobals& r) {
    ...
    r.drawAimLineTargets = j.value("drawAimLineTargets", r.drawAimLineTargets);
    ...
    r.markerScale = std::clamp(j.value("markerScale", r.markerScale), 0.75f, 2.0f);
...
void from_json(const nlohmann::json& j, atlasVisibilityGlobals& v)
{
    ...
    v.maxDistance = std::clamp(
        j.value("maxDistance", v.maxDistance),
        10,
        500);
}
...
void to_json(nlohmann::json& j, const keyGlobals& k) {
    j = nlohmann::json{
        ...
        {"toggleRadarMinView", k.toggleRadarMinView}
    };
}
...
void from_json(const nlohmann::json& j, keyGlobals& k) {
    ...
    k.toggleRadarMinView = j.value("toggleRadarMinView", k.toggleRadarMinView);
}
...
void to_json(nlohmann::json& j, const lootGlobals& k) {
    j = nlohmann::json{
        ...
        {"containerDistance", k.containerDistance},
...
void from_json(const nlohmann::json& j, lootGlobals& k) {
    ...
    k.containerDistance = std::clamp(
        j.value("containerDistance", k.containerDistance),
        10,
        1000);
...
bool ConfigManager::LoadConfig()
{
    ...
    try
    {
        ...
        if (j.contains("atlasVisibilityGlobals") &&
            j["atlasVisibilityGlobals"].is_object())
        {
            atlasVisibility_ =
